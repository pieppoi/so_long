...
void	ft_check_rows(t_game *game)
{
	...
	while (i < game->map.rows)
	{
		if (game->map.full[i][0] != WALL)
			ft_error_msg(
				"Invalid Map. Missing wall at first row.",
				game);
		else if (game->map.full[i][game->map.columns - 1] != WALL)
			ft_error_msg(
				"Invalid Map. Missing wall at last row.",
				game);
		i++;
	}
}
...
void	ft_check_columns(t_game *game)
{
	...
	while (i < game->map.columns)
	{
		if (game->map.full[0][i] != WALL)
			ft_error_msg(
				"Invalid Map. Missing wall at first column.",
				game);
		else if (game->map.full[game->map.rows - 1][i] != WALL)
			ft_error_msg(
				"Invalid Map. Missing wall at last column.",
				game);
		i++;
	}
}
	...
	while (y < game->map.rows)
	{
		...
		while (x < game->map.columns)
		{
			if (!ft_strchr("CEP01", game->map.full[y][x]))
				ft_error_msg("Invalid Map. Unexpected parameter.", game);
...
void	ft_verify_map_parameters(t_game *game)
{
	if (game->map.coins == 0)
		...
		ft_error_msg(
			"Invalid Map. Invalid Player quantity."
